...
test('accepts user-authorized partial or full contacts access on iOS', () => {
  ...
  assert.match(cardPackageSource, /请选择需要授权的联系人，完成后继续签署。/)
  assert.doesNotMatch(cardPackageSource, /推荐选择“完全访问”/)
  assert.doesNotMatch(cardPackageSource, /iOS 18 及以上可以选择部分联系人/)
  assert.doesNotMatch(cardPackageSource, /只会上传您授权的联系人/)
  assert.match(cardPackageSource, /contacts-full-access-dialog/)
  assert.doesNotMatch(cardPackageSource, /继续选择授权方式/)
  assert.match(cardPackageSource, /confirmButtonText: '继续授权'/)
  assert.match(cardPackageSource, /已上传您授权的联系人/)
  assert.match(cardPackageSource, /已取消授权，未上传任何联系人/)
  assert.doesNotMatch(cardPackageSource, /系统不会上传任何联系人/)
  assert.match(cardPackageSource, /未获取到含手机号的已授权联系人/)
  assert.match(cardPackageSource, /返回卡包后稍后重试/)
  assert.doesNotMatch(cardPackageSource, /通过网页签署或联系客服处理/)
  assert.match(infoPlist, /<key>NSContactsUsageDescription<\/key>/)
  assert.match(infoPlist, /仅在您主动申请的先享后付订单审核通过并进入合同签署前/)
  assert.match(infoPlist, /用于读取您的授权/)
  assert.doesNotMatch(infoPlist, /必须选择完全访问/)
})
