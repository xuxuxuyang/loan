...
<template>
  <section class="wv-bg-login-page min-h-screen px-5 py-10">
    <div class="relative mx-auto w-full max-w-[390px] p-5">
      <div class="mb-6 flex items-center gap-3">
        <div class="wv-gradient-amber-icon flex h-11 w-11 items-center justify-center rounded-xl shadow-[0_6px_14px_rgba(221,146,25,0.24)]">
          <Icon
      ...
      <div class="mb-4 flex rounded-full border border-black/[0.06] bg-white/90 p-1 shadow-sm">
        <button
          ...
          :class="loginMode === 'sms' ? 'wv-gradient-primary-soft login-tab-active' : 'wv-text-muted'"
          @click="loginMode = 'sms'"
        >
          验证码登录
        </button>
        <button
          type="button"
          class="flex-1 rounded-full py-2 text-sm font-semibold transition"
          :class="loginMode === 'password' ? 'wv-gradient-primary-soft login-tab-active' : 'wv-text-muted'"
      ...
      <div class="space-y-3">
        ...
        <template v-if="loginMode === 'sms'">
          <div class="flex items-center gap-2.5">
            ...
            <button
              type="button"
              class="wv-gradient-primary-soft h-[52px] shrink-0 rounded-full px-4 text-sm font-semibold transition hover:brightness-105 disabled:opacity-70"
      ...
      <button
        type="button"
        class="wv-gradient-primary login-submit mt-7 w-full rounded-full py-3 text-[1.5rem] font-semibold transition hover:brightness-105 disabled:opacity-60"
...
.login-tab-active {
  box-shadow: 0 4px 12px rgba(235, 112, 137, 0.35);
}

.login-submit {
  box-shadow: 0 10px 20px rgba(235, 112, 137, 0.22);
}
</style>
