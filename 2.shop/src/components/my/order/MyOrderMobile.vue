...
watch(
  [() => String(route.query.status ?? 'all'), currentUserPhone],
  ([statusRaw, phone]) => {
    if (import.meta.env.SSR || !phone) {
      return
    }
    const status = statusTabs.some(item => item.key === statusRaw)
      ? statusRaw as 'all' | MallOrderStatus
      : 'all'
    void syncFromRemote(phone, { status })
  },
  { immediate: true },
)
...
async function onPaySuccess() {
  ...
  if (phone) {
    await syncFromRemote(phone, { status: activeStatus.value })
  }
}
</script>
