...
function submitAddCard() {
  ...
  if (!bankName) {
    notifyWarning('请输入银行名称')
    return
  }
  if (!addForm.cardType.trim()) {
    notifyWarning('请选择银行卡类型')
    return
  }
  if (cardDigits.length < 12 || cardDigits.length > 19) {
    notifyWarning('请输入12-19位银行卡号')
    return
  }
  if (!owner) {
    notifyWarning('请输入持卡人姓名')
    return
  }

  if (!currentUserAccount.value) {
    notifyWarning('请先登录后再管理银行卡')
    return
  }
  adding.value = true
  createBankCard(currentUserAccount.value, {
    ...
    closeAddDialog()
    notifySuccess('银行卡添加成功')
  }).catch((error) => {
    console.error('新增银行卡失败', error)
    notifyError('添加失败，请稍后重试')
  }).finally(() => {
    adding.value = false
  })
}
...
async function handleDeleteCard(item: BankCardItem) {
  if (!currentUserAccount.value) {
    notifyWarning('请先登录后再管理银行卡')
    return
  }
...
<template>
  <section class="px-4 pb-5 pt-4">
    ...
    <article
      v-for="item in cards"
      :key="item.id"
      class="relative mb-3 overflow-hidden rounded-2xl p-4 text-white shadow-[0_10px_24px_rgba(26,55,99,0.16)]"
      :class="pickCardTheme(cards.indexOf(item))"
    >
      <button
        type="button"
        class="absolute right-3 top-3 flex h-8 w-8 items-center justify-center rounded-full bg-white/20 text-white backdrop-blur-sm transition hover:bg-white/30 disabled:opacity-50"
      ...
      <p class="mb-3 pr-10 text-sm text-white/85">
        {{ item.bankName }}
      </p>
