...
function pruneStaleEntries() {
  ...
  for (const [phone, row] of loginSmsByPhone.entries()) {
    if (now > row.expiresAt + 120_000) {
      loginSmsByPhone.delete(phone)
    }
  }
}
...
function buildLoginSmsMsg(code) {
  let tpl = String(process.env.MALL_LOGIN_SMS_MSG_TEMPLATE || '').trim()
  const signBracket = String(process.env.MALL_REGISTER_SMS_SIGN_BRACKET || '').trim()

  if (!tpl) {
    const sign = signBracket || '宁波海曙文硕'
    tpl = `【${sign}】您的登录验证码为{code}，5分钟内有效`
  }
  else if (!/【[^】]+】/.test(tpl) && signBracket) {
    tpl = `【${signBracket}】${tpl}`
  }
  ...
  if (!tpl.includes('{code}')) {
    const err = new Error('登录短信模板须包含占位符 {code}（环境变量 MALL_LOGIN_SMS_MSG_TEMPLATE）')
    err.httpStatus = 503
    throw err
  }
  return tpl.split('{code}').join(code)
...
/**
 * 向已注册手机号发送登录验证码（经风控上游 clSms/send）。
 * @param {string} phone 11 位国内手机号
 */
async function sendLoginVerificationSms(phone) {
  pruneStaleEntries()
  const now = Date.now()
  const prev = loginSmsByPhone.get(phone)
  if (prev && now - prev.lastSentAt < RESEND_MS) {
    const err = new Error('发送过于频繁，请稍后再试')
    err.httpStatus = 429
    throw err
  ...
  const code = generateCode()
  const msg = buildLoginSmsMsg(code)
  ...
  if (!ch.ok) {
    const text = describeClSmsUpstreamFailure(res, ch.reason)
    console.warn('[mallRegisterSms] login clSms/send rejected', { phone, upstream: res && res.json })
    const err = new Error(text)
    err.httpStatus = res && Number(res.status) >= 400 && Number(res.status) < 600 ? Number(res.status) : 502
    throw err
  }

  loginSmsByPhone.set(phone, {
    code,
    expiresAt: now + TTL_MS,
    lastSentAt: now,
  })
 ...
 * @returns {{ ok: true } | { ok: false, reason: string }}
 */
function verifyAndConsumeLoginSms(phone, inputCode) {
  const want = String(inputCode || '').trim()
  if (!/^\d{6}$/.test(want)) {
    return { ok: false, reason: '请输入 6 位短信验证码' }
  }
  const row = loginSmsByPhone.get(phone)
  if (!row) {
    return { ok: false, reason: '请先获取短信验证码' }
  }
  if (Date.now() > row.expiresAt) {
